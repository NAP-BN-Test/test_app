...
import styles from "./ListVideo.css";
...
const ListVideo = (props: any) => {
  const params = props.route.params.channelID;
  ...
  const miniCardData: any = useSelector((state: RootState) => state.video);
  ...
  async function toggleGetListVideoFromChannel() {
    setIsLoading(true);
    await dispatch(
      Action.act_getListVideoFromChannel(params, nextPageToken, value)
    );
    setIsLoading(false);
  }
  ...
  const handleLoadMore = async () => {
    if (!onEndReachedCalledDuringMomentum) {
      console.log("load");
      await Services.getListVideoFromChannel(
        params,
      ...
      <View style={styles.body}>
        ...
        <View style={styles.back}>
          <Ionicons
            ...
            size={28}
            color="tomato"
            onPress={toggleGetListVideoFromChannel}
          />
        </View>
      </View>
